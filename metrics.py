...
from utils_sssnet import scipy_sparse_to_torch_sparse
...
class Prob_Balanced_Normalized_Loss(torch.nn.Module):
    ...
    def __init__(self, A_p, A_n):
        ...
        D_p = sp.diags(A_p.transpose().sum(axis=0).tolist(), [0]).tocsc()
        D_n = sp.diags(A_n.transpose().sum(axis=0).tolist(), [0]).tocsc()
        self.D_bar = scipy_sparse_to_torch_sparse(D_p + D_n)
        mat = (D_p - A_p) + A_n
        self.mat = scipy_sparse_to_torch_sparse(mat)


        A_p_power = A_p.dot(A_p)
        D_p_power = sp.diags(A_p_power.transpose().sum(axis=0).tolist(), [0]).tocsc()
        A_n_power = A_p.dot(A_n)
        # A_n_power = 0.5 * (A_p.dot(A_n) + A_n.dot(A_p))
        D_n_power = sp.diags(A_n_power.transpose().sum(axis=0).tolist(), [0]).tocsc()
        self.D_bar_power = scipy_sparse_to_torch_sparse(D_p_power + D_n_power)
        mat_power = (D_p_power - A_p_power) + A_n_power
        self.mat_power = scipy_sparse_to_torch_sparse(mat_power)
    ...
    def forward(self, prob):
        ...
        mat_power = self.mat_power.to(device)
        D_bar_power = self.D_bar_power.to(device)
        ...
        for k in range(prob.shape[-1]):
            ...
            denominator_power = torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(D_bar_power, prob_vector_mat))[0, 0] + epsilon  # avoid dividing by zero
            numerator_power = (torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(mat_power, prob_vector_mat)))[0, 0]

            result += numerator - 0.3 * denominator # 0.3
            # result += 1 * (numerator_power - 0.3 * denominator_power)
            # result += (numerator - 0.3 * denominator) + 0.3 * (numerator_power - 0.3 * denominator_power)

        return result/prob.shape[0]

# N-500: numerator - 0.01 * denominator, (5,1,5,1)
# N-1000: numerator - 0.05 * denominator, (3,1,3,1)/(5,1,5,1);
# N-1000, eta-0.05: numerator - 0.05 * denominator, (1,1,1,1)
# N-1500: numerator - 0.1 * denominator, (1,1,1,1)/(2,1,2,1)  # smooth越强，越需要防止collapse。smooth 可以用指标衡量：
...
class imp_Prob_Balanced_Normalized_Loss(torch.nn.Module):
    r"""An implementation of the probablistic balanced normalized cut loss function.
    Args:
        A_p, A_n (scipy sparse matrices): positive and negative parts of adjacency matrix A.
    """
    ...
    def __init__(self, A_p, A_n):
        super(imp_Prob_Balanced_Normalized_Loss, self).__init__()
        D_p = sp.diags(A_p.transpose().sum(axis=0).tolist(), [0]).tocsc()
        D_n = sp.diags(A_n.transpose().sum(axis=0).tolist(), [0]).tocsc()
        self.D_bar = scipy_sparse_to_torch_sparse(D_p + D_n)
        # mat = (D_p - A_p) + A_n
    ...
    def forward(self, prob):
        ...
        D_bar = self.D_bar.to(device)
        result = torch.zeros(1).to(device)
        # print('mean_confidence', torch.mean(torch.max(prob, dim=1)[0]))

        for k in range(prob.shape[-1]):
            prob_vector_mat = prob[:, k, None]

            denominator = torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(D_bar, prob_vector_mat))[
                              0, 0] + epsilon  # avoid dividing by zero
            numerator = (torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(mat, prob_vector_mat)))[0, 0]

            result += -numerator / denominator
        return result

class min_intra_variance(torch.nn.Module):
    r"""An implementation of the probablistic balanced normalized cut loss function.
    Args:
        A_p, A_n (scipy sparse matrices): positive and negative parts of adjacency matrix A.
    """
    ...
    def __init__(self, num_cluster):
        super(min_intra_variance, self).__init__()
        self._num_clusters = int(num_cluster)

    def forward(self, z, pred_clu):
        """Making a forward pass of the probablistic balanced normalized cut loss function.
        Args:
            prob: (PyTorch FloatTensor) Prediction probability matrix made by the model
        ...
        Returns:
            loss value.
        """
        pred_one_hot_label = F.one_hot(pred_clu, num_classes=self._num_clusters)
        for i in range(self._num_clusters):
            all_z_i = pred_one_hot_label[:,i].unsqueeze(1) * z
            cen_i = torch.mean(all_z_i, dim=0)
            print(cen_i.size())
            print(all_z_i - cen_i)
            print(all_z_i[0] - cen_i)
            dist = all_z_i - cen_i
            z_dist = torch.sum(torch.mul(dist, dist)) / (self.features.shape[0] * self.num_clusters)
            print(torch.log(z_dist))
...
#             numerator = (torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(mat,prob_vector_mat)))[0,0]
#
#             result += numerator - 0.001*denominator # 大作用 或者 去掉denominator
#         return result
...
#         mat = (D_p - A_p) + A_n
#         L_p = (D_p - A_p)
#         self.A_n = scipy_sparse_to_torch_sparse(A_n)
#         self.mat = scipy_sparse_to_torch_sparse(mat)
#
#     def forward(self, prob):
#         """Making a forward pass of the probablistic balanced normalized cut loss function.
...
#         mat = self.mat.to(device)
#         D_bar = self.D_bar.to(device)
#         A_n = self.A_n.to(device)
#         result = torch.zeros(1).to(device)
#         with torch.no_grad():
#             pred_labels = torch.argmax(prob, dim=1)
#             one_hot_pred_labels = torch.nn.functional.one_hot(pred_labels, num_classes=prob.size()[1])
#         denominators, numerators = 0, 0
#         for k in range(prob.size()[1]):
#             with torch.no_grad():
#                 k_pred_label = one_hot_pred_labels[:, k].unsqueeze(1)
#                 k_prob = k_pred_label * prob
#             prob_vector_mat = k_prob[:, k, None]
#
#
#             denominators = torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(mat, prob_vector_mat))[0,0] + epsilon
#             numerators += (torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(A_n, prob_vector_mat)))[0,0]
#
...
#         D_bar = self.D_bar.to(device)
#         A_n = self.A_n.to(device)
#         # print('D_bar, mat', type(D_bar), type(mat))
#         result = torch.zeros(1).to(device)
#         with torch.no_grad():
#             pred_labels = torch.argmax(prob, dim=1)
#             one_hot_pred_labels = torch.nn.functional.one_hot(pred_labels, num_classes=prob.size()[1])
...
#         result = torch.zeros(1).to(device)
#         with torch.no_grad():
#             mask = (prob == prob.max(dim=1, keepdim=True)[0]).to(dtype=torch.int32).detach()
#             k_prob = torch.mul(mask, prob)
#         denominators, numerators = 0, 0
#         for k in range(prob.size()[1]):
#             # k_pred_label = one_hot_pred_labels[:, k].unsqueeze(1)
#             # k_prob = k_pred_label * prob
...
#             denominator = torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(D_bar, prob_vector_mat))[0, 0] + epsilon  # avoid dividing by zero
#             numerator = (torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(mat, prob_vector_mat)))[0, 0]
#             # numerator = (torch.matmul(torch.transpose(prob_vector_mat, 0, 1), torch.matmul(mat_power, prob_vector_mat)))[0, 0]
#
#             result += numerator / denominator
#         return result
...
class Prob_Balanced_Ratio_Loss(torch.nn.Module):
    ...
    def forward(self, prob):
        ...
        for k in range(prob.shape[-1]):
            prob_vector_mat = prob[:, k, None]
            denominator = (torch.matmul(torch.transpose(prob_vector_mat, 0, 1), prob_vector_mat) + 1)[
...
def real_data_analysis(dataset='wikirfa', result_folder_name='../result_arrays/wikirfa/',
    ...
    print_performance_mean_std(dataset, np.concatenate(
        (cut, 100 * unhappy_ratios[:, :, None], size_ratio[:, :, None], size_std[:, :, None]), 2), compare_names_all,
                               metric_names)
